...
// Inode table
static pthread_rwlock_t *ilocks; // inode table locks
...
static pthread_mutex_t datablocks_lock; // lock for data blocks
...
static pthread_mutex_t open_filetable_lock; // lock for the open file table
...
// Auxiliary functions to be called in operations.c if needed and to improve
// readability
void lock_write_itable() {
    ALWAYS_ASSERT(pthread_rwlock_wrlock(ilocks) == 0, "read lock\n");
}
void lock_read_itable() {
    ALWAYS_ASSERT(pthread_rwlock_rdlock(ilocks) == 0, "read lock\n");
}
void unlock_itable() {
    ALWAYS_ASSERT(pthread_rwlock_unlock(ilocks) == 0, "read lock\n");
}
void lock_datablocks() {
    ALWAYS_ASSERT(pthread_mutex_lock(&datablocks_lock) == 0, "mutex lock\n");
}
void unlock_datablocks() {
    ALWAYS_ASSERT(pthread_mutex_unlock(&datablocks_lock) == 0,
                  "thread unlock\n");
}
void lock_open_file_table() {
    ALWAYS_ASSERT(pthread_mutex_lock(&open_filetable_lock) == 0,
                  "mutex lock\n");
}
void unlock_open_file_table() {
    ALWAYS_ASSERT(pthread_mutex_unlock(&open_filetable_lock) == 0,
                  "thread unlock\n");
}
void lock_inode_creation() {
    ALWAYS_ASSERT(pthread_mutex_lock(&inode_creation_lock) == 0,
                  "mutex lock\n");
}
void unlock_inode_creation() {
    ALWAYS_ASSERT(pthread_mutex_unlock(&inode_creation_lock) == 0,
                  "thread unlock\n");
}
...
int state_init(tfs_params params) {
    ...
    pthread_mutex_init(&datablocks_lock, NULL);
    pthread_mutex_init(&open_filetable_lock, NULL);
    // inode lock table has same size as inode table size
    ilocks = malloc(INODE_TABLE_SIZE * sizeof(pthread_rwlock_t));
    ...
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        freeinode_ts[i] = FREE;
        // initializes all locks of the inode locks table
        ALWAYS_ASSERT(pthread_rwlock_init(&ilocks[i], NULL) == 0,
                      "error initializing inode lock");
    }
...
int state_destroy(void) {
    // destroys all locks from the inode locks table
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        ALWAYS_ASSERT(pthread_rwlock_destroy(&ilocks[i]) == 0,
                      "error destroying inode lock");
    }
    free(ilocks);
    free(inode_table);
    free(freeinode_ts);
    ...
    pthread_mutex_destroy(&datablocks_lock);
    pthread_mutex_destroy(&open_filetable_lock);
    inode_table = NULL;
    freeinode_ts = NULL;
    fs_data = NULL;
    free_blocks = NULL;
...
int inode_create(inode_type i_type) {
    ...
    case T_DIRECTORY: {
        ...
        if (b == -1) { // allocation failed, deletes
 ...
 */
int add_dir_entry(inode_t *inode, char const *sub_name,
                  int sub_inumber) { // FIXME adicionar sincronização
    ...
    // Finds and fills the first empty entry, unlocking afterwards
    for (size_t i = 0; i < MAX_DIR_ENTRIES; i++) {
    ...
    // if there are no open entries, we still need to unlock
    unlock_datablocks();
 ...
 */
int find_in_dir(inode_t const *inode,
                char const *sub_name) { // FIXME experimentar sincronização
...
int data_block_alloc(void) {
    ...
    for (size_t i = 0; i < DATA_BLOCKS; i++) {
        ...
        if (free_blocks[i] == FREE) {
            // free block found: update and unlock
            free_blocks[i] = TAKEN;
            unlock_datablocks();
    ...
    // there is a chance that no block is free, and we still need to unlock
    unlock_datablocks();
    return -1;
}
...
int add_to_open_file_table(int inumber, size_t offset) {
    // while we search for the file, we don't want the table to change
    lock_open_file_table();
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (free_open_file_entries[i] == FREE) {
            // found slot to add an open file: writes and unlocks after
    ...
    // in the case we didn't find a slot, we still need to unlock
    unlock_open_file_table();
    return -1;
}
...
/**
 * Search if a given file is already open.
 *
 * Input:
 *  - inumber: inode number of the file to search
 *
 * Returns file handle if the file is already open, -1 otherwise.
 */
int search_in_open_file_table(int inumber) {
    lock_open_file_table();
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
